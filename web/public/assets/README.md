...
- 已订阅员工接口统一返回 `avatarAsset`（人物 ID、内容版本、主图与头肩图绝对 URL）；Web 和客户端按返回值选构图。接入约定见 [员工头像共享协议](../../../docs/对接/员工头像共享协议.md)。
...
5. 新人物更新映射/生成清单，并运行 `pnpm assets:sync` 更新后端头像注册表（主图和头肩图内容共同决定版本）；`pnpm assets:check` 检查一致性。其他资产登记 `visual-assets.json`，含稳定 id、src、alt；新资产组更新本 README 的用途、尺寸和数量。
