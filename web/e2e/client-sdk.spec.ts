...
test.describe('客户端 SDK 核心流程', () => {
  ...
  test('1. 获取企业 API 密钥', async ({ request }) => {
    ...
    enterpriseId = loginData.data.user.enterprise.id;

    // 创建 API 密钥
    const keyResponse = await request.post(
      `http://localhost:4000/api/enterprises/${enterpriseId}/settings/api-keys`,
      {
        headers: {
  ...
  test('2. 设备登录', async ({ request }) => {
    ...
    const response = await request.post('http://localhost:4000/api/client/auth/device-login', {
      headers: {
        'x-api-key': apiKey,
      },
      data: {
  ...
  test('3. 模型网关 - Chat Completion', async ({ request }) => {
    ...
    const response = await request.post('http://localhost:4000/api/client/gateway/chat/completions', {
      headers: {
  ...
  test('5. 查询余额和用量', async ({ request }) => {
    ...
    // 查询计算配额余额
    const balanceResponse = await request.get('http://localhost:4000/api/client/compute/balance', {
      headers: {
        Authorization: `Bearer ${deviceToken}`,
      },
    });
    ...
    // 查询用量记录
    const usageResponse = await request.get('http://localhost:4000/api/client/compute/usage?days=7', {
      headers: {
        Authorization: `Bearer ${deviceToken}`,
      },
    });
  ...
  test('6. 设备登出', async ({ request }) => {
    ...
    const response = await request.post('http://localhost:4000/api/client/auth/logout', {
      headers: {
        Authorization: `Bearer ${deviceToken}`,
      },
    });
    ...
    // 验证 token 已失效
    const verifyResponse = await request.get('http://localhost:4000/api/client/compute/balance', {
      headers: {
        Authorization: `Bearer ${deviceToken}`,
      },
    });
