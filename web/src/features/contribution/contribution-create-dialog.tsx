...
export function ContributionCreateDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const reset = () => {
    setStep(1); setType('skill'); setName(''); setDescription(''); setIndustry(''); setPosition(''); setContent(''); setPlatform('coze'); setWorkflowUrl('');
  };
  const close = (value: boolean) => { if (!value) reset(); onOpenChange(value); };
  const canContinue = step === 1 || (step === 2 && name.trim().length > 0 && description.trim().length >= 10);

  const next = () => {
    if (step === 2 && description.trim().length < 10) { toast.error('能力说明至少需要 10 个字'); return; }
  ...
  return <Dialog open={open} onOpenChange={close}>
    <DialogContent glass className="max-w-3xl overflow-hidden p-0">
      <DialogHeader className="border-b border-glassline px-6 py-5">
        ...
        <div className="mt-5 flex items-center gap-2" aria-label="创建步骤"><StepIndicator step={1} current={step} label="选择类型" /><StepLine active={step > 1} /><StepIndicator step={2} current={step} label="能力信息" /><StepLine active={step > 2} /><StepIndicator step={3} current={step} label="执行配置" /></div>
      </DialogHeader>
      <div className="max-h-[min(560px,calc(100vh-230px))] overflow-y-auto px-6 py-5 scroll-thin">
        ...
        {step === 2 && <InfoStep name={name} description={description} industry={industry} position={position} onNameChange={setName} onDescriptionChange={setDescription} onIndustryChange={setIndustry} onPositionChange={setPosition} />}
        {step === 3 && <ConfigStep type={type} content={content} platform={platform} workflowUrl={workflowUrl} onContentChange={setContent} onPlatformChange={setPlatform} onWorkflowUrlChange={setWorkflowUrl} />}
      </div>
...
function InfoStep({ name, description, industry, position, onNameChange, onDescriptionChange, onIndustryChange, onPositionChange }: { name: string; description: string; industry: string; position: string; onNameChange: (value: string) => void; onDescriptionChange: (value: string) => void; onIndustryChange: (value: string) => void; onPositionChange: (value: string) => void }) {
  return <div><SectionIntro eyebrow="02 / 能力信息" title="让别人一眼理解它的价值" description="清晰的名称和适用范围会直接影响企业审核与后续复用。" /><div className="mt-6 grid gap-4"><label className="block text-sm text-gtext-secondary">能力名称<Input glass value={name} onChange={(event) => onNameChange(event.target.value)} placeholder="例如：竞品周报生成器" className="mt-1.5" /></label><label className="block text-sm text-gtext-secondary">能力说明<Textarea value={description} onChange={(event) => onDescriptionChange(event.target.value)} placeholder="说明它解决什么问题、适用什么场景，至少 10 个字" className="mt-1.5 min-h-24 border-glassline bg-glass-2 text-gtext-primary placeholder:text-gtext-muted focus-visible:ring-gbrand-ring" /></label><div className="grid gap-4 sm:grid-cols-2"><label className="block text-sm text-gtext-secondary">适用行业<span className="mb-1 block text-[11px] text-gtext-muted">多个标签用逗号分隔</span><Input glass value={industry} onChange={(event) => onIndustryChange(event.target.value)} placeholder="例如：互联网、软件服务" /></label><label className="block text-sm text-gtext-secondary">适用岗位<span className="mb-1 block text-[11px] text-gtext-muted">多个标签用逗号分隔</span><Input glass value={position} onChange={(event) => onPositionChange(event.target.value)} placeholder="例如：研发负责人、项目经理" /></label></div></div></div>;
}

function ConfigStep({ type, content, platform, workflowUrl, onContentChange, onPlatformChange, onWorkflowUrlChange }: { type: ContributionType; content: string; platform: 'coze' | 'dify' | 'n8n' | 'opencode'; workflowUrl: string; onContentChange: (value: string) => void; onPlatformChange: (value: 'coze' | 'dify' | 'n8n' | 'opencode') => void; onWorkflowUrlChange: (value: string) => void }) {
  return <div><SectionIntro eyebrow="03 / 执行配置" title={type === 'skill' ? '写下它的工作方法' : '接入它的执行入口'} description={type === 'skill' ? '建议包含角色、输入、步骤、输出和边界条件，系统会在提交审核前自动校验。' : '保存外部工作流地址后，系统会在审核阶段检查接入信息。'} /><div className="mt-6">{type === 'skill' ? <label className="block text-sm text-gtext-secondary">Skill 模板<Textarea value={content} onChange={(event) => onContentChange(event.target.value)} placeholder={'# 角色\n# 输入\n# 步骤\n# 输出\n# 边界条件'} className="mt-1.5 min-h-56 border-glassline bg-glass-2 font-mono text-xs leading-6 text-gtext-primary placeholder:text-gtext-muted focus-visible:ring-gbrand-ring" /><span className="mt-2 block text-[11px] text-gtext-muted">{content.trim().length} / 至少 20 个字符</span></label> : <div className="grid gap-4 sm:grid-cols-2"><label className="block text-sm text-gtext-secondary">执行平台<select value={platform} onChange={(event) => onPlatformChange(event.target.value as typeof platform)} className="mt-1.5 h-10 w-full rounded-glass-md border border-glassline bg-glass-2 px-3 text-sm text-gtext-primary outline-none focus:border-glassline-brand"><option value="coze">Coze</option><option value="dify">Dify</option><option value="n8n">n8n</option><option value="opencode">OpenCode</option></select></label><label className="block text-sm text-gtext-secondary">工作流地址<Input glass value={workflowUrl} onChange={(event) => onWorkflowUrlChange(event.target.value)} placeholder="https://..." className="mt-1.5" /><span className="mt-2 block text-[11px] text-gtext-muted">需要可访问的 HTTPS 地址</span></label></div>}</div></div>;
}
