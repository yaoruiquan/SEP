...
describe('AllocateComputeDialog', () => {
  ...
  it('弹窗里必须有确定按钮 —— 只有「取消」等于这个功能不存在', () => {
    ...
    fireEvent.change(screen.getByLabelText('每月限额（元）'), {
      target: { value: '500' },
    });
  ...
  it('留空 = 不限额，按钮文案跟着变，提交 null', () => {
    ...
    expect(setAllowance).toHaveBeenCalledWith(
      expect.objectContaining({ userId: 'user-1', limitCNY: null, dailyLimitCNY: null, monthlyLimitCNY: null }),
      expect.anything(),
    );
  });
  ...
  it('快捷额度按钮把金额填进输入框', () => {
    ...
    expect(screen.getByLabelText('每月限额（元）')).toHaveValue(500);
  });
