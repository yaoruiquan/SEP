...
export const buttonVariants = cva(
  'inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md font-medium transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background disabled:pointer-events-none disabled:opacity-50 active:scale-[0.98]',
  {
    variants: {
      variant: {
        primary: 'bg-primary text-white hover:bg-primary-hover shadow-sm hover:shadow',
        // 中性次级按钮：走语义 token，暗色玻璃作用域下自动取深色实心表面，
        // 不再依赖 globals.css 的 SCOPED OVERRIDES 兜 bg-white/neutral-*。
        secondary:
          'bg-card text-foreground border border-border hover:bg-muted hover:border-border',
        outline:
          'bg-transparent text-foreground border border-border hover:bg-muted hover:border-border',
